...
# Additional experiment configurations to try
experiments = [
    {
        "name": "ultra_low_lr",
        "batch_size": 16,
...
"""
        ...
        if exp.get("modify_config", False):
            if 'return TransformerConfig(' in line:
                ...
                # For the balanced_capacity experiment, use more heads and hidden size
                if exp["name"] == "balanced_capacity":
                    hidden_size = 192
                    num_heads = 6
                else:
                    hidden_size = 128
                ...
                modified_config = f"""        return TransformerConfig(
...
# Save final results
with open(f"experiment_results/additional_summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json", 'w') as f:
