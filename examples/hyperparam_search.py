...
def perform_hyperparameter_search():
    ...
    for lr_h in lr_hidden_candidates:
        for scale_base in inference_lr_scale_base_candidates:
            ...
            # <<< --- END ADD --- >>>
                    ...
                    wandb_run_name = f"{nb_str}_lrw{lr_w:.0e}_lrh{lr_h:.0e}_{scale_base_str}_{h_clip_str}_{w_clip_str}_e{fixed_overrides['epochs']}"

                    print(f"Parameters: lr_h={lr_h}, scale={scale_base}, h_clip={h_clip}, w_clip={w_clip}, epochs={fixed_overrides['epochs']}")
                    ...
                    try:
                        ...
                        if is_valid_mse:
                            if final_mse < best_run_info["mse"]:
                                ...
                                print(f"*** New best MSE: {final_mse:.6f} with Params: lr_h={lr_h}, scale={scale_base}, h_clip={h_clip}, w_clip={w_clip} ***")
                        ...
                        print(f"!!!! Run {current_run} failed with Params: lr_h={lr_h}, scale={scale_base}, h_clip={h_clip}, w_clip={w_clip}. Error: {e} !!!!")
                        # <<< --- END UPDATE --- >>>
                        import traceback
                        traceback.print_exc()
    ...
    with open(log_file_path, 'w') as f:
        ...
        f.write("-------------------------------------------------------------------------------------------------\n")
        # <<< --- UPDATE LOG HEADER --- >>>
        f.write("Run | LR Hid | Scale | H Clip | W Clip | W&B Run Name                                 | Final Train MSE | Status\n")
        # <<< --- END UPDATE --- >>>
        f.write("-------------------------------------------------------------------------------------------------\n")
        ...
        for result in all_run_results:
            ...
            f.write(f"{result['run_number']:<3} | {result['lr_hidden']:<6} | {result['inference_lr_scale_base']:<5} | {h_clip_str:<6} | {w_clip_str:<6} | {run_name_str:<46} | {mse_str:<15} | {status_str:<6}\n")
            # <<< --- END UPDATE --- >>>
        f.write("-------------------------------------------------------------------------------------------------\n")
        if best_run_info["lr_hidden"] is not None:
            ...
            f.write(f"Best Params: peak_lr_hidden={best_run_info['lr_hidden']}, inference_lr_scale_base={best_run_info['inference_lr_scale_base']}, h_grad_clip_norm={h_clip_best_str}, w_grad_clip_norm={w_clip_best_str} (lr_weights={best_run_info['lr_weights']} fixed)\n")
